...
@section('content')
    ...
    <div class='bg-red-200 min-h-screen flex flex-col items-center justify-center'>
        ...
        <form action="/login" method='POST' class='text-xl flex flex-col items-center mb-4'>
            @csrf
            ...
            <button type='submit' class='bg-gray-500 p-3 text-white hover:bg-gray-800'>Login</button>
        </form>

        @if (session('status'))
            <div class='bg-red-800 p-2 rounded text-center text-white'>
                {{ session ('status') }}
            </div>
        @endif
