...
@section('content')
    ...
    <div class='bg-red-200 min-h-screen flex items-center flex-col'>
        ...
        <div class='flex flex-col items-center w-full mt-2 p-3'>
            <h1 class='text-5xl p-5 mb-6 text-center'>Le Nostre Pizze</h1>
            <div class='flex items-center flex-col sm:flex-row sm:items-start p-2'>
                <div class='order-2 sm:order-1 sm:mr-4 w-44'> 
                    <button class='category-select-btn p-1 border border-black rounded bg-gray-300 w-full' id='category-select-btn'>All Categories v</button>
                    <div style='overflow-wrap: break-word' class='hidden flex flex-col max-h-15 bg-gray-300 border border-black rounded max-h-32 overflow-y-auto overflow-x-visible' id='categories-list'>
        ...
        @if( count($pizzas) > 0)
            @foreach($pizzas as $pizza)
                <div class='border border-3 border-black rounded-lg mb-3 p-5 flex flex-col items-center justify-center'>
                    ...
                    <a class='mb-4' href="/pizzas/cat/{{ $pizza->category->slug }}"> ( {{ $pizza->category->name }} ) </a>
                </div>  
            @endforeach
        @else 
